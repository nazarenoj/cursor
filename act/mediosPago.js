...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (cajasParaAsociar.length > 0) {
      const valores = cajasParaAsociar.map(cajaId => [result.insertId, cajaId]);
      await query(
        'INSERT INTO medio_pago_cajas (medio_pago_id, caja_id) VALUES ?',
        [valores]
      );
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (cajasIds !== undefined || cajaId !== undefined) {
      ...
      if (cajasParaAsociar.length > 0) {
        const valores = cajasParaAsociar.map(cajaId => [id, cajaId]);
        await query(
          'INSERT INTO medio_pago_cajas (medio_pago_id, caja_id) VALUES ?',
          [valores]
        );
